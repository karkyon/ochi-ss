...
exports.startBizPoller = startBizPoller;
...
async function processNewNotifications() {
  ...
  `);
    ...
    for (const notif of notifications) {
        ...
        if (success) {
            await pool.request()
                .input("通知ID", mssql_1.default.UniqueIdentifier, notif.通知ID)
                .query(`
          UPDATE WEB進捗通知
...
async function handleNumberAssigned(notif) {
    if (!notif.WEB見積ID) {
    ...
    if (order) {
        ...
        await prisma.notification.create({
            data: {
                ...
                notifType: "info",
                targetCustomers: client_1.Prisma.JsonNull,
...
async function handleStatusChange(order, notif) {
    await prisma.$transaction([
        ...
        prisma.notification.create({
            data: {
                ...
                notifType: "info",
                targetCustomers: client_1.Prisma.JsonNull, // TODO: 対象得意先を絞る
                publishedAt: new Date(),
                createdBy: "sync_service",
            },
        }),
...
async function handleSpecChange(order, notif) {
    await prisma.$transaction([
        // 仕様変更履歴を追記
...
async function handleTrackingUpdate(order, notif) {
    await prisma.order.update({
        where: { id: order.id },
...
//  ヘルパー
// ---------------------------------------------------------------------------
function resolveTargetTable(notifType) {
    switch (notifType) {
