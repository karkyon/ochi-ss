...
// ---------------------------------------------------------------------------
async function startOutboxPoller(queue) {
    const poll = async () => {
        try {
            ...
            for (const event of pendingEvents) {
                ...
                await queue.add("send-to-biz", { eventId: event.id }, {
                    jobId: event.id,
...
// ---------------------------------------------------------------------------
function createOutboxWorker(redisConnection) {
    return new bullmq_1.Worker(OUTBOX_QUEUE, async (job) => {
        ...
        const pool = await (0, sqlserver_1.getSqlServerPool)();
...
// ---------------------------------------------------------------------------
async function processOutboxEvent(event, pool) {
...
async function syncEstimateToConfirmTables(estimateHeaderId, pool) {
    ...
    // --- ヘッダー MERGE ---
    await pool.request()
        .input("WEB見積ID", mssql_1.default.NVarChar(36), header.id)
        .input("WEB見積番号", mssql_1.default.NVarChar(20), header.estimateNo ?? null)
        .input("得意先コード", mssql_1.default.NVarChar(10), header.customerCode)
        .input("得意先名", mssql_1.default.NVarChar(100), header.customerName)
        .input("得意先担当者名", mssql_1.default.NVarChar(50), header.chargeName ?? null)
        .input("お客様注文番号", mssql_1.default.NVarChar(50), header.customerOrderNo ?? null)
        .input("エンドユーザー番号", mssql_1.default.NVarChar(50), header.endUserNo ?? null)
        .input("入力日付", mssql_1.default.Date, header.inputDate)
        .input("見積日付", mssql_1.default.Date, header.estimateDate ?? null)
        .input("見積合計金額", mssql_1.default.Money, header.details.reduce((s, d) => s + Number(d.totalPrice ?? 0), 0))
        .input("明細件数", mssql_1.default.Int, header.details.length)
        .query(`
    ...
    for (const d of header.details) {
        ...
        const destDeliveryId = useIndiv ? d.indivDirectDeliveryId ?? null : header.directDeliveryId ?? null;
        ...
        await pool.request()
            .input("WEB見積明細ID", mssql_1.default.NVarChar(36), d.id)
            .input("WEB見積ID", mssql_1.default.NVarChar(36), header.id)
            .input("行番号", mssql_1.default.Int, d.rowNo)
            .input("材料コード", mssql_1.default.NVarChar(4), d.materialCode)
            .input("材料名", mssql_1.default.NVarChar(20), d.materialName ?? null)
            .input("加工仕様コード", mssql_1.default.Int, d.kakouShiyouCode)
            .input("加工仕様", mssql_1.default.NVarChar(10), d.kakouShiyou ?? null)
            .input("加工指示コードT", mssql_1.default.NVarChar(10), d.kakouShijiCodeT ?? null)
            .input("加工指示コードA", mssql_1.default.NVarChar(10), d.kakouShijiCodeA ?? null)
            .input("加工指示コードB", mssql_1.default.NVarChar(10), d.kakouShijiCodeB ?? null)
            .input("仕上りサイズT", mssql_1.default.Decimal(7, 3), d.sizeT)
            .input("仕上りサイズA", mssql_1.default.Decimal(7, 3), d.sizeA)
            .input("仕上りサイズB", mssql_1.default.Decimal(7, 3), d.sizeB)
            .input("加工公差_UT", mssql_1.default.Decimal(6, 3), d.kousaTUpper ?? null)
            .input("加工公差_LT", mssql_1.default.Decimal(6, 3), d.kousaTLower ?? null)
            .input("加工公差_UA", mssql_1.default.Decimal(6, 3), d.kousaAUpper ?? null)
            .input("加工公差_LA", mssql_1.default.Decimal(6, 3), d.kousaALower ?? null)
            .input("加工公差_UB", mssql_1.default.Decimal(6, 3), d.kousaBUpper ?? null)
            .input("加工公差_LB", mssql_1.default.Decimal(6, 3), d.kousaBLower ?? null)
            .input("面取り量_4C", mssql_1.default.Decimal(5, 2), d.mentori4 ?? null)
            .input("面取り量_8C", mssql_1.default.Decimal(5, 2), d.mentori8 ?? null)
            .input("製品数量", mssql_1.default.Int, d.quantity)
            .input("見積単価", mssql_1.default.Money, d.unitPrice ?? null)
            .input("見積金額", mssql_1.default.Money, d.totalPrice ?? null)
            .input("最短納期", mssql_1.default.VarChar(10), d.shortestDelivery ?? null)
            .input("納期有効期限", mssql_1.default.DateTime2, d.deliveryDeadline ?? null)
            .input("客先注番", mssql_1.default.NVarChar(50), header.customerOrderNo ?? null)
            .input("WEB直送先ID", mssql_1.default.NVarChar(36), destDeliveryId)
            .input("直送先名_WEB入力", mssql_1.default.NVarChar(100), destName ?? null)
            .input("直送先郵便番号_WEB", mssql_1.default.NVarChar(10), destZip ?? null)
            .input("直送先住所_WEB", mssql_1.default.NVarChar(255), destAddr ?? null)
            .input("直送先電話番号_WEB", mssql_1.default.NVarChar(15), destTel ?? null)
            .query(`
