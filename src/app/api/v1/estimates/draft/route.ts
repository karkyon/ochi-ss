...
export async function POST(req: NextRequest) {
  ...
  let body: { headerPartial?: Record<string, unknown>; details?: unknown[]; isDraftOnly?: boolean }
  ...
  try {
    const header = await withTenant(ctx.customerId, ctx.isSuperAdmin, async (tx) => {
      const h = await (tx as any).estimateHeader.create({
        data: {
          estimateNo: null, estimateStatus: "draft", isDraftOnly: true,
          draftExpiresAt, draftSavedAt: now, draftDeviceInfo,
          customerId: ctx.customerId, customerCode: ctx.companyCode ?? "",
          customerName: "", createdBy: ctx.userId ?? "",
      ...
      if (Array.isArray(body.details) && body.details.length > 0) {
        ...
        await (tx as any).estimateDetail.createMany({
          data: details.map((d, i) => ({
            estimateHeaderId: h.id, rowNo: i + 1,
    ...
    return NextResponse.json(
      { estimateId: header.id, draftSavedAt: header.draftSavedAt?.toISOString(), draftExpiresAt: header.draftExpiresAt?.toISOString() },
      { status: 201 }
    )
  } catch (err: unknown) {
