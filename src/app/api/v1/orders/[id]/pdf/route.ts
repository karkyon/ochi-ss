...
// 2026/07/16 デザイン刷新: 見積書PDF(estimates/[id]/pdf/route.ts)と統一感のある
// A4帳票レイアウト・配色・カード意匠に全面刷新。会社情報も管理画面(SystemSetting)
// から取得するよう統一する。
...
export async function GET(
  ...
  // 自社情報（管理画面 /admin/company で編集可能。未設定時は既定値を使用。見積書PDFと統一）
  ...
  const subtotal = details.reduce((s: number, d: any) => s + Number(d.totalPrice ?? 0), 0)
  const taxRate = 0.10
  const tax = Math.round(subtotal * taxRate)
  const grandTotal = subtotal + tax

  // 公差フォーマットヘルパー（+上限/-下限）。見積書PDFと同一ロジック。
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .status-pill {
      display: inline-block; font-size: 10px; font-weight: 700; padding: 1px 8px; border-radius: 999px;
      background: #e0e7ff; color: #3730a3;
    }

    /* 合計金額サマリーカード（見積書PDFと統一） */
    .summary-banner {
      display: flex; align-items: center; justify-content: space-between;
...
<body>
  ...
  <div class="sheet">
    ...
    <div class="doc-title-bar">
      <div class="doc-title">注　文　書<small>PURCHASE ORDER</small></div>
      <div class="doc-no-box">
        注文No：<span class="estno">${order.orderNo ?? "—"}</span><br>
        注文日：${orderDate}<br>
    ...
    <div class="top-grid">
      <div class="dest-box">
        ...
        <div class="sub" style="margin-top:8px; color:#475569;">
          下記の通り注文を受け付けいたしました。内容をご確認くださいませ。
        </div>
      </div>
      <div class="info-card">
        <h3>注文情報</h3>
        ...
        <div class="info-row"><span class="label">注文状況</span><span class="value"><span class="status-pill">${statusLabel}</span></span></div>
    ...
    <div class="totals-wrap">
      <table class="totals-table">
        <tr><td>注文金額小計（税抜）</td><td>¥${subtotal.toLocaleString()}</td></tr>
        <tr><td>消費税（10%）</td><td>¥${tax.toLocaleString()}</td></tr>
        <tr class="grand"><td>ご注文合計金額</td><td>¥${grandTotal.toLocaleString()}</td></tr>
      </table>
    </div>
