...
export default async function DashboardPage() {
  ...
  try {
    ...
    const rows = await prisma.notification.findMany({
      where: { isDeleted: false, isActive: true, OR: [{ publishedAt: null }, { publishedAt: { lte: now } }], OR: [{ expiresAt: null }, { expiresAt: { gt: now } }] },
    ...
    const reads = customerId
      ? await prisma.notificationRead.findMany({ where: { customerId, notificationId: { in: rows.map(r => r.id) } }, select: { notificationId: true } })
      : []
    const readSet = new Set(reads.map(r => r.notificationId))
    notifications = rows.map(r => ({ ...r, isRead: readSet.has(r.id) }))
    unreadCount = notifications.filter(n => !n.isRead).length
  } catch { /* サイレント */ }

  const cardStyle = (c: typeof MENU_CARDS[0]): React.CSSProperties => ({
    ...
    <div style={{ minHeight: "100vh", background: "#f1f5f9" }}>
      ...
      <div style={{ maxWidth: "960px", margin: "0 auto", padding: "16px" }}>
        ...
        <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: "8px", padding: "10px 14px", marginBottom: "14px" }}>
          ...
          <div style={{ fontSize: "10px", color: "#94a3b8" }}>得意先コード：{(session.user as any).customerCode ?? ""}</div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", marginBottom: "14px" }}>
          {MENU_CARDS.map(card => (
            <Link key={card.id} href={card.href} style={cardStyle(card)}>
